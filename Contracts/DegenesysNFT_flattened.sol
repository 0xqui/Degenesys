...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function getAvailableTokenAtIndex(uint256 indexToUse, uint updatedNumAvailableTokens)
        ...
        returns (uint256)
    {
        uint256 result = _availableTokens[indexToUse];
...
// File: contracts/DegenesysNFT.sol
...
//   /_.'/_'/_//_'/ //_'_\/_/_\ 
//          _/            _/    
//  Twitter: @DegenesysNFT
//  https://degenesys.xyz
//                                              

/* 
* This contract enables pseudo-random minting of NFTs.
* This is designed to inhibit the ability of rarity snipers and ensure a fairer mint.
*
* Only EOA's are able to mint (no contracts).
*
* While using more gas than the ERC721A standard, the ERC721r (random) standard
* Uses a modern Fisher-Yates shuffle to minimise the computational expense - https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_modern_algorithm.
* On Ethereum mainnet this trade-off is likely not worth it, and developers should explore
* Other methods of randomization or fairness.
*
* h/t to Tom Lehman (middlemarch.eth) for work on the ERC721r contract, which is lightly modified to allow tokenIds to begin at 1 instead of 0.
*
* The contract also has team mint, whitelist mint and an ownerClaimed function to enable minting at specific index (honoraries etc).
* The mintAtIndex function is inherited from erc721r, and can only be used prior to the start of random minting - hence it's called in construction here. 
*/
...
contract DegenesysNFT is ERC721r, Ownable, ReentrancyGuard {
  Turnstile turnstile = Turnstile(0xEcf044C5B4b867CFda001101c617eCd347095B44);

  using Strings for uint256;

  string public uriPrefix = 'ipfs://bafybeiasthlnaylhdydomtbqm5sryzteylx235cqibo6zjskdshtmrbe5m/';
  string public uriSuffix = '.json';

  uint256 public maxPublicMintAmountPerTx = 3; 
  uint256 public maxTeamMintAmountPerWallet = 10; 
  uint256 public maxWhitelistMintAmountPerWallet = 3;

  uint256 public publicMintCost = 200 ether;
  uint256 public teamMintCost = 0 ether;
  uint256 public whitelistMintCost = 180 ether;
  ...
  function setteamList(address[] calldata addresses) external onlyOwner {
    for(uint256 i=0; i <addresses.length; i++){
      teamList[addresses[i]] = true;
    }
  }


// mint 1/1s for Build-A-DGEN winners
  function ownerClaimed() internal {
    _mintAtIndex(msg.sender, 112);
    _mintAtIndex(msg.sender, 193);
    _mintAtIndex(msg.sender, 432);
  }



  function teamMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(teamMintEnabled, 'The team sale is not enabled!');
    require(teamList[_msgSender()], "You are not on the Team List");
  ...
  function whitelistMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) nonReentrant {
    require(whitelistMintEnabled, 'The whitelist is not enabled!');
    require(whiteList[_msgSender()], "You are not on the White List");
